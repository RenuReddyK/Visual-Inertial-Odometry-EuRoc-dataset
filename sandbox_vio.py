...
main_data_dir = "/Users/renu/Desktop/Upenn/Sem_2/MEAM620/HWs/HW2/Project 2_3/meam620/proj2_3/dataset/MachineHall01_reduced/"
...
#%%
w_b_all = np.array(w_b_all).reshape((-1,3))
plt.figure()
plt.plot(w_b_all[:,0], label='wx')
plt.plot(w_b_all[:,1], label='wy')
plt.plot(w_b_all[:,2], label='wz')
plt.ylabel('meters per second squared')
plt.title('Gyroscope Bias (w_b)')
plt.legend()
plt.show()

#%%
g_all = np.array(g_all).reshape((-1,3))

plt.figure()
plt.plot(g_all[:,0], label='gx')
plt.plot(g_all[:,1], label='gy')
plt.plot(g_all[:,2], label='gz')
plt.ylabel('meters per second squared')
plt.title('Gravity (g)')
plt.legend()
plt.show()
